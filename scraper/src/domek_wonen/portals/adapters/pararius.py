...
class _ParariusHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "section" and "search-list__item" in class_name:
            self._card = {}
        ...
        field_map = {
            "listing-search-item__link": "property_url",
            "listing-search-item__title": "address_raw",
            "listing-search-item__location": "city_raw",
            "listing-search-item__price": "price_raw",
            "listing-search-item__features--area": "living_area_raw",
            "listing-search-item__features--rooms": "rooms_raw",
            "listing-search-item__features--property-type": "property_type_raw",
            "listing-search-item__brand": "broker_raw",
            "listing-search-item__label": "status_raw",
            "source-evidence": "source_evidence",
        }

        if tag == "img":
            self._card["image_url"] = attributes.get("src", "")
        if tag == "a" and "listing-search-item__link" in class_name:
...
def build_search_url(city: str) -> str:
    return f"https://www.pararius.nl/koopwoningen/{quote_plus(normalize_text(city).lower())}"


def parse_listing_cards(html: str, city_query: str, search_url: str, page_number: int) -> list[PortalListing]:
    parser = _ParariusHTMLParser()
    parser.feed(html)
